...
#show: project.with(
  title: "Flex",
  subtitle: "3nd laboratory report",
  authors: (
...
In previous assignments, we explored the theoretical foundations of regular languages
and automata. The next step is to apply these concepts to the construction of real
compiler tools. In this assignment, we focus on lexical analysis, the first stage of a
compiler, where a stream of characters from an input program is transformed into a
sequence of meaningful tokens.

The goal of this practice is to implement a lexical analyzer (scanner) using *Flex*.
By defining token patterns through regular expressions, the scanner will be capable of
recognizing elements of a subset of the C language such as identifiers, numeric
literals, keywords, and operators. The program will read a source file and produce a
structured list of tokens along with their corresponding lexemes.

== Problem Statement

Given an input file containing code written in a subset of the C language, the program must:

1. Implement a lexical analyzer using *Flex* that recognizes tokens through regular expressions.
2. Identify and classify language elements such as keywords, identifiers, numeric literals,
   and operators.
3. Correctly ignore whitespace and comments using Flex states when necessary.
4. Output the recognized tokens in the format `[TOKEN:LEXEME]` to allow automated validation.
