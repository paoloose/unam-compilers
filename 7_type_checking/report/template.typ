  ...
  show heading: it => {
      if it.level == 1 and it.numbering != none {
        if features.contains("full-page-chapter-title") {
          ...
          full-page-chapter.update(false)
          v(16pt)
          // text(size: 20pt)[#dict.chapter #counter(heading).display(): #it.body ]
          text(size: 20pt)[#counter(heading).display(). #it.body ]
          v(1pt)
        }
      } else {
        full-page-chapter.update(false)
